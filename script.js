...
async function visualizeModelLayers(model) {
  ...
  model.layers.forEach((layer, layerIndex) => {
    ...
    if (layerType === 'Dense') neuronCount = config.units;
      ...
      neuronCount = Math.min(neuronCount, 64);
    }
    if (!neuronCount) return;

    createNeuronsForLayer(layerIndex, neuronCount, layerType, layer.name);
  });
}
...
function createNeuronsForLayer(layerIndex, neuronCount, layerType, layerName) {
  ...
  for (let i = 0; i < neuronCount; i++) {
    ...
    sphere.position.y = 50 - layerIndex * 18;
...
function drawDenseConnections(neuron) {
  ...
  for (let i = 0; i < nextCount; i++) {
    ...
    const points = [neuron.position, target.position];
...
function normalizeFilter(filter) {
  ...
  if (max === min) {
    return filter.map(row => row.map(() => 127));
  }
...
function preprocessDrawingForModel(canvasEl) {
  return tf.tidy(() => {
    const base = tf.browser.fromPixels(canvasEl, 1).toFloat();
    const normalized = base.div(255);
...
function setupDrawingTools() {
  ...
  predictButton.onclick = () => {
    ...
    showTopPredictions(prediction.squeeze());
    input.dispose();
    prediction.dispose();
...
async function run() {
  ...
  setBusyState(false);
  setStatus('Ready. Use Inspect mode to hover nodes and drawing tools to predict digits.');
...
// Small deterministic check for normalization edge-case behavior.
